...
import { LayoutDashboard, Users, Bell, Settings, LogOut, Search, Sun, Moon, Menu, X, ShoppingCart, Globe, Camera, FileText, UserCog, Heart, Sparkles, ClipboardList, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* Sidebar */}
      <aside className={`${sidebarWidth} h-screen fixed top-0 start-0 z-[999] bg-sidebar text-sidebar-foreground flex flex-col transition-all duration-300 overflow-y-auto overflow-x-hidden ${isSidebarOpen ? 'translate-x-0' : 'max-lg:-translate-x-full rtl:max-lg:translate-x-full'}`}>
        {/* Logo */}
        <div className={`flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-3 px-5'} h-16 border-b border-white/10 shrink-0`}>
          ...
          {!isSidebarCollapsed && (
            <span className="text-base font-extrabold tracking-tight text-white truncate">{settings.studioName.split(' ')[0]}</span>
          )}
      ...
      {/* Main */}
      <main className={`flex-1 ${mainMargin} min-h-screen flex flex-col w-full overflow-x-hidden transition-all duration-300`}>
        <header className="h-16 bg-card/80 backdrop-blur-xl border-b border-border flex items-center justify-between px-5 lg:px-7 sticky top-0 z-[900]">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div onClick={() => setActiveNav(10)} className="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white font-bold text-xs ms-1 cursor-pointer hover:scale-105 transition-transform shadow-md shadow-primary/20">{user.name.charAt(0).toUpperCase()}</div>
