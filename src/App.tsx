      ...
      {days.map((day, i) => (
        <div key={day} className="flex-1 flex flex-col items-center gap-2.5 h-full">
          <div className="flex gap-1 items-end flex-1 w-full">
            <motion.div
              className="flex-1 rounded-t-md bg-gradient-to-t from-sky-600 to-sky-500"
              initial={{ height: 0 }}
            ...
            />
            <motion.div
              className="flex-1 rounded-t-md bg-sky-200 dark:bg-sky-500/20"
              initial={{ height: 0 }}
              animate={{ height: `${(values2[i] / max) * 100}%` }}
              transition={{ delay: i * 0.1 + 0.05, duration: 0.6 }}
              style={{ minHeight: 4 }}
            />
          </div>
...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
    ...
    () => [
      {
        icon: Users,
        title: t.activeUsers,
        value: stats ? (stats.activeUsers || 0).toLocaleString() : "...",
        change: `${stats?.totalUsers || 0} ${lang === "ar" ? "مستخدم نظام" : "sys users"}`,
        positive: true,
        color: "#0ea5e9",
        data: [30, 40, 35, 50, 49, 60, 70, 91, 85, stats?.activeUsers || 0],
      },
      {
        icon: ShoppingCart,
        title: t.totalOrders,
        value: stats ? (stats.totalOrders || 0).toLocaleString() : "...",
        change: `+${stats?.admins || 0} ${lang === "ar" ? "مدير" : "admins"}`,
        positive: true,
        color: "#10b981",
        data: [20, 25, 30, 28, 35, 40, 38, 42, 45, stats?.totalOrders || 0],
      },
      {
        icon: DollarSign,
        title: t.revenue,
        value: stats ? (stats.revenue || 0).toLocaleString() : "...",
        change: lang === "ar" ? "إجمالي الأرباح" : "Total Revenue",
        positive: true,
        color: "#f59e0b",
        data: [45, 52, 49, 55, 60, 58, 65, 70, 68, stats?.revenue || 0],
      },
      {
        icon: TrendingUp,
        title: t.conversion,
        value: stats ? (stats.balance || 0).toLocaleString() : "...",
        change: `${stats?.bannedUsers || 0} ${lang === "ar" ? "غير نشط" : "inactive"}`,
        positive: (stats?.bannedUsers || 0) === 0,
        color: "#ef4444",
        data: [5, 4, 5, 4, 4, 3, 4, 3, 2, stats?.balance || 0],
      },
    ],
    [stats, lang, t],
  );
  ...
  const activities = useMemo(
    () =>
      stats?.recentUsers?.map((u: any) => ({
        icon: UserPlus,
        text: u.name,
        time: new Date(u.created_at).toLocaleDateString(lang === "ar" ? "ar-EG" : "en-US", {
          month: "short",
          day: "numeric",
          hour: "2-digit",
          minute: "2-digit",
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-2 sm:mb-3">
                      ...
                      <button className="text-muted-foreground hover:text-foreground p-1 rounded-lg hover:bg-muted transition-all hidden sm:block">
                        <MoreHorizontal size={16} />
                      </button>
                    </div>
                    <p className="text-[10px] sm:text-xs font-medium text-muted-foreground mb-1 sm:mb-1.5 truncate">
                      {stat.title}
                    </p>
                    <div className="flex items-end justify-between gap-2 sm:gap-3 mb-1.5 sm:mb-2.5">
                      <h3 className="text-lg sm:text-2xl font-extrabold text-foreground tracking-tight leading-none">
                        {stat.value}
                      </h3>
                    ...
                    <span
                      className={`inline-flex items-center gap-1 text-[10px] sm:text-xs font-semibold px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full ${stat.positive ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}
                    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] xl:grid-cols-[1fr_360px] gap-4 sm:gap-5 mb-5">
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5">
              <div>
                <h3 className="font-bold text-foreground text-sm">{t.salesOverview}</h3>
              ...
              <div className="flex gap-0.5 bg-muted p-0.5 rounded-lg">
                {(["weekly", "monthly", "yearly"] as const).map((p) => (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-md text-xs font-semibold transition-all ${chartPeriod === p ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
                  >
                    {t[p]}
                  </button>
                ))}
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex justify-between items-center mb-5">
              ...
              <button className="flex items-center gap-1 text-primary text-xs font-semibold hover:bg-primary/5 px-2 py-1 rounded-lg transition-all">
                {t.viewAll}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-foreground truncate">{act.text}</p>
                      <p className="text-xs text-muted-foreground">{act.time}</p>
                    </div>
                    <button className="text-muted-foreground opacity-0 group-hover:opacity-100 hover:bg-muted p-1.5 rounded-lg transition-all">
                      <Eye size={15} />
                    </button>
                  </motion.div>
                );
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex justify-between items-start mb-5">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {lang === "ar" ? "آخر المستخدمين المسجلين في النظام" : "Latest registered users"}
                </p>
              </div>
              <button
                onClick={() => setActiveNav(1)}
                className="flex items-center gap-1 text-primary text-xs font-semibold hover:bg-primary/5 px-2 py-1 rounded-lg transition-all"
              >
                {t.viewAll}
                <ChevronRight size={14} />
              </button>
                ...
                <thead>
                  <tr className="border-b border-border">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-4 py-2.5 text-start text-[11px] font-bold text-muted-foreground uppercase tracking-wider"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {stats.recentUsers.map((u: any, idx: number) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div
                            ...
                            style={{
                              background: `${u.role === "admin" ? "#ef4444" : u.role === "editor" ? "#f59e0b" : "#0ea5e9"}15`,
                            }}
                          >
                            <UserPlus
                              size={15}
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${u.role === "admin" ? "bg-red-500/10 text-red-500" : "bg-success/10 text-success"}`}
                        >
                          {u.role === "admin"
                            ? lang === "ar"
                              ? "مدير"
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${u.status === "active" ? "bg-success/10 text-success" : "bg-red-500/10 text-red-500"}`}
                        >
                          {u.status === "active"
                            ? lang === "ar"
                              ? "نشط"
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* Sidebar */}
      <aside
        className={`w-[260px] h-screen fixed top-0 start-0 z-[999] bg-sidebar text-sidebar-foreground flex flex-col p-5 transition-transform duration-300 overflow-y-auto ${isSidebarOpen ? "translate-x-0" : "max-lg:-translate-x-full rtl:max-lg:translate-x-full"}`}
      >
        <div className="flex items-center gap-3 px-2.5 mb-7">
          <div className="w-9 h-9 bg-gradient-to-br from-sky-500 to-cyan-400 rounded-xl flex items-center justify-center text-white shadow-lg shadow-sky-500/30">
            <Aperture size={20} />
          </div>
          <span className="text-lg font-extrabold tracking-tight text-white">{settings.studioName.split(" ")[0]}</span>
          <button
            className="lg:hidden ms-auto text-slate-400 hover:text-white p-1 rounded-lg hover:bg-white/10 transition-all"
            onClick={() => setSidebarOpen(false)}
          >
            <X size={20} />
          </button>
              ...
              return (
                <li key={item.key}>
                  <button
                    ...
                    className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-start ${activeNav === idx ? "bg-white/10 text-white font-semibold" : "text-slate-400 hover:bg-white/5 hover:text-slate-200"}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${activeNav === idx ? "bg-gradient-to-br from-sky-500 to-cyan-400 text-white shadow-md shadow-sky-500/25" : ""}`}
                    >
                      <Icon size={18} />
                    </div>
        ...
        <div className="mt-auto pt-4 border-t border-white/10 space-y-2.5">
          <div className="flex items-center gap-3 px-2.5">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-sky-400 to-cyan-400 flex items-center justify-center text-white font-bold text-xs shrink-0">
          ...
          <button
            onClick={onLogout}
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border border-red-500/20 bg-red-500/5 text-red-400 text-xs font-semibold hover:bg-red-500/10 transition-all"
          >
            <LogOut size={16} />
            {t.logout}
          </button>
      ...
      <main className="flex-1 lg:ms-[260px] min-h-screen flex flex-col w-full overflow-x-hidden">
        <header className="h-16 bg-card/80 backdrop-blur-xl border-b border-border flex items-center justify-between px-5 lg:px-7 sticky top-0 z-[900]">
          ...
          <div className="flex items-center gap-1.5">
            <button
              onClick={toggleLang}
              className="h-9 px-3 rounded-lg border border-border bg-card text-muted-foreground text-xs font-bold hover:bg-muted hover:text-foreground transition-all flex items-center gap-1.5"
            >
              <Globe size={16} />
            ...
            <button
              onClick={toggleTheme}
              className="h-9 w-9 rounded-lg border border-border bg-card text-muted-foreground hover:bg-muted hover:text-foreground transition-all flex items-center justify-center"
            >
              {theme === "dark" ? <Sun size={17} /> : <Moon size={17} />}
            </button>
            <button
              onClick={() => setNotificationsOpen(true)}
              className="h-9 w-9 rounded-lg border border-border bg-card text-muted-foreground hover:bg-muted hover:text-foreground transition-all flex items-center justify-center relative"
            >
              <Bell size={17} />
              <span className="absolute top-1.5 end-1.5 w-2 h-2 bg-red-500 rounded-full border-2 border-card" />
            </button>
            <div
              onClick={() => setActiveNav(9)}
              className="w-9 h-9 rounded-full bg-gradient-to-br from-sky-500 to-cyan-400 flex items-center justify-center text-white font-bold text-xs ms-1 cursor-pointer hover:scale-105 transition-transform shadow-md shadow-sky-500/20"
            >
              {user.name.charAt(0).toUpperCase()}
            </div>
