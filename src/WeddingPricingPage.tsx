...
import { Heart, Plus, Edit3, Trash2, DollarSign, X, Save, AlertCircle, Loader, BookOpen, Video, Camera, Image, Ruler, Package } from 'lucide-react';
import { useSettings } from './SettingsContext';
import { getWeddingAlbums, createWeddingAlbum, updateWeddingAlbum, deleteWeddingAlbum, getWeddingVideos, createWeddingVideo, updateWeddingVideo, deleteWeddingVideo, getInventoryItems } from './api';

interface Album { id: number; description: string; price: number; photo_count: number; size: string; color: string; inventory_item_id?: number; product_name?: string; }
interface VideoPricing { id: number; camera_type: string; quality: string; price_per_hour: number; color: string; }
interface InventoryItem { id: number; item_name: string; sell_price: number; usage_type: string; }
...
const translations = {
  ar: {
    ...
    linkProduct: 'ربط بمنتج من المخزون', noProduct: 'بدون منتج مرتبط', selectProduct: 'اختر منتجاً...',
  },
  en: {
    ...
    linkProduct: 'Link to Inventory Product', noProduct: 'No linked product', selectProduct: 'Select product...',
  },
};
...
const WeddingPricingPage: React.FC = () => {
  ...
  const [aInvItem, setAInvItem] = useState<number | undefined>(undefined);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [a, v, i] = await Promise.all([getWeddingAlbums(), getWeddingVideos(), getInventoryItems()]);
      setAlbums(a.data);
      setVideos(v.data);
      setInventory(i.data.filter((item: any) => item.usage_type === 'wedding'));
  ...
  const openAlbumModal = (a?: Album) => {
    if (a) { setEditAlbum(a); setADesc(a.description); setAPrice(a.price); setAPhotos(a.photo_count); setASize(a.size || ''); setAInvItem(a.inventory_item_id); }
    else { setEditAlbum(null); setADesc(''); setAPrice(0); setAPhotos(0); setASize(''); setAInvItem(undefined); }
    setAlbumModal(true);
  };
  const saveAlbum = async () => {
    const data = { description: aDesc, price: aPrice, photo_count: aPhotos, size: aSize, inventory_item_id: aInvItem, color: editAlbum?.color || COLORS[albums.length % COLORS.length] };
          ...
          {albums.length === 0 ? (
            ...
            <motion.div key={album.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08 }}
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                ...
                {album.product_name && <div className="flex items-center gap-2"><Package size={14} style={{ color: album.color }} /><span className="text-xs">{album.product_name}</span></div>}
              </div>
            </motion.div>
          ))}
        </div>
            ...
            <motion.div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl overflow-hidden" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} onClick={e => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><label className="block text-xs font-semibold text-foreground mb-1.5">{t.linkProduct}</label><div className="flex items-center gap-2.5 bg-muted border border-border rounded-lg px-3.5 h-11 focus-within:border-pink-500/50 focus-within:ring-2 focus-within:ring-pink-500/10"><Package size={16} className="text-muted-foreground" /><select value={aInvItem || ''} onChange={e => setAInvItem(e.target.value ? Number(e.target.value) : undefined)} className="flex-1 bg-transparent border-none outline-none text-foreground text-sm font-cairo cursor-pointer"><option value="">{t.noProduct}</option>{inventory.map(item => <option key={item.id} value={item.id}>{item.item_name} ({item.sell_price} {cur})</option>)}</select></div></div>
                </div>
              </div>
