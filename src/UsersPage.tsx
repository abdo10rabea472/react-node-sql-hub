...
const UsersPage: React.FC = () => {
  ...
  const openEditModal = (user: UserData) => { setFormName(user.name); setFormEmail(user.email); setFormPassword(''); setFormRole(user.role); setFormStatus(user.status); setFormBaseSalary(String(user.base_salary || 0)); setFormShiftStart(user.shift_start || '09:00'); setFormShiftEnd(user.shift_end || '17:00'); setEditingUser(user); };

  const handleAdd = async () => { if (!formName || !formEmail || !formPassword) return; setSaving(true); try { await createUser({ name: formName, email: formEmail, password: formPassword, role: formRole, shift_start: formShiftStart, shift_end: formShiftEnd } as any); showToast(l.success, 'success'); setShowAddModal(false); fetchUsers(); } catch (err: any) { showToast(err.response?.data?.message || l.errorOp, 'error'); } finally { setSaving(false); } };
            ...
            <motion.div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl overflow-hidden" initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={e => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="bg-muted/50 rounded-xl p-4 border border-border/50">
                  <label className="block text-xs font-bold text-foreground mb-3 flex items-center gap-2">🕐 {l.shiftSchedule}</label>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="block text-[10px] font-semibold text-muted-foreground mb-1">{l.shiftStart}</label><input type="time" value={formShiftStart} onChange={e => setFormShiftStart(e.target.value)} className="w-full px-3 py-2 bg-card border border-border rounded-lg text-foreground text-sm outline-none focus:border-primary/50 font-cairo" /></div>
                    <div><label className="block text-[10px] font-semibold text-muted-foreground mb-1">{l.shiftEnd}</label><input type="time" value={formShiftEnd} onChange={e => setFormShiftEnd(e.target.value)} className="w-full px-3 py-2 bg-card border border-border rounded-lg text-foreground text-sm outline-none focus:border-primary/50 font-cairo" /></div>
                  </div>
                </div>
              </div>
