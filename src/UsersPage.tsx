...
interface UserData { id: number; name: string; email: string; role: string; status: string; created_at: string; updated_at: string; }

const t = {
  ar: { title: 'إدارة المستخدمين', subtitle: 'عرض وإدارة جميع المستخدمين في النظام', addUser: 'إضافة مستخدم', search: 'ابحث بالاسم أو البريد...', name: 'الاسم', email: 'البريد الإلكتروني', password: 'كلمة المرور', role: 'الدور', status: 'الحالة', actions: 'إجراءات', joinDate: 'تاريخ الانضمام', admin: 'مدير', editor: 'محرر', user: 'مستخدم', active: 'نشط', inactive: 'غير نشط', banned: 'محظور', edit: 'تعديل', delete: 'حذف', save: 'حفظ', cancel: 'إلغاء', confirmDelete: 'هل أنت متأكد من حذف هذا المستخدم؟', yes: 'نعم، احذف', no: 'لا، إلغاء', noUsers: 'لا يوجد مستخدمين', total: 'المجموع', addUserTitle: 'إضافة مستخدم جديد', editUserTitle: 'تعديل المستخدم', allRoles: 'جميع الأدوار', allStatuses: 'جميع الحالات', loading: 'جاري التحميل...', errorLoad: 'خطأ في تحميل البيانات', retry: 'إعادة المحاولة', success: 'تمت العملية بنجاح', errorOp: 'حدث خطأ' },
  en: { title: 'User Management', subtitle: 'View and manage all system users', addUser: 'Add User', search: 'Search by name or email...', name: 'Name', email: 'Email', password: 'Password', role: 'Role', status: 'Status', actions: 'Actions', joinDate: 'Join Date', admin: 'Admin', editor: 'Editor', user: 'User', active: 'Active', inactive: 'Inactive', banned: 'Banned', edit: 'Edit', delete: 'Delete', save: 'Save', cancel: 'Cancel', confirmDelete: 'Are you sure you want to delete this user?', yes: 'Yes, Delete', no: 'No, Cancel', noUsers: 'No users found', total: 'Total', addUserTitle: 'Add New User', editUserTitle: 'Edit User', allRoles: 'All Roles', allStatuses: 'All Statuses', loading: 'Loading...', errorLoad: 'Error loading data', retry: 'Retry', success: 'Operation successful', errorOp: 'An error occurred' },
};
...
const UsersPage: React.FC = () => {
  ...
  const openAddModal = () => { setFormName(''); setFormEmail(''); setFormPassword(''); setFormRole('user'); setFormStatus('active'); setShowAddModal(true); };
  const openEditModal = (user: UserData) => { setFormName(user.name); setFormEmail(user.email); setFormPassword(''); setFormRole(user.role); setFormStatus(user.status); setEditingUser(user); };
  ...
  const handleEdit = async () => { if (!editingUser) return; setSaving(true); try { const data: Record<string, string> = { name: formName, email: formEmail, role: formRole, status: formStatus }; if (formPassword) data.password = formPassword; await updateUser(editingUser.id, data); showToast(l.success, 'success'); setEditingUser(null); fetchUsers(); } catch (err: any) { showToast(err.response?.data?.message || l.errorOp, 'error'); } finally { setSaving(false); } };
